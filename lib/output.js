 ...
 * @property {number} maxPathLength - Max path string length
 * @property {number} rangePadding - Length of padding before range
 * @property {number} pathPadding - Length of padding before range
 */
...
/**
 * Default number of columns in terminal
 */
const DEFAULT_TTY_COLUMNS = 180;
...
const getPaddingLengths = (deps) => {
  ...
  const maxPathLength =
    (process.stdout.columns || DEFAULT_TTY_COLUMNS) -
    pathPadding;
...
const getPathsString = (dep, paddingLengths) => {
  const {paths} = dep;
  const {
    pathPadding,
    maxPathLength
  ...
  const prefixString = `\n${Array(pathPadding).fill(' ').join('')}`;

  return paths.map((path, pathIndex) => {
    const pathReverse = path.slice().reverse();
    const prefix = pathIndex === 0 ? '' : prefixString;
    let pathString = '';

    pathReverse.forEach((parent, parentIndex) => {
      if (pathString <= maxPathLength) {
        pathString += prefixString;
      }
